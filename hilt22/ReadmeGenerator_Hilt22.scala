...
import org.sireum.cli.hamr_runners.hilt22.ReadmeTemplate.createHyperLink
...
import org.sireum.hamr.ir.{GclSubclause, JSON => irJSON, MsgPack => irMsgPack}
import org.sireum.message.{Position, Reporter}

@record class ReadmeGenerator_Hilt22(o: Cli.SireumHamrCodegenOption, reporter: Reporter) {
  ...
  def genJVMRunInstructions(readmeDir: Os.Path,
                            osireumScript: Option[Os.Path],
                            outputDir: Os.Path): ST = {

    val projectcmd = outputDir / "bin" / "project.cmd"
    assert(projectcmd.exists)

    val loc = readmeDir.relativize(projectcmd)
    val link = createHyperLink(loc.value, loc.value)
    val ret: ST =
      st"""See the instructions in the generated Proyek project definition file $link
...
object ReadmeTemplate {
  ...
  def getGumboPos(sc: GclSubclause): Option[Position] = {
    var pos: Option[Position] = None()
    for(stateVar <- sc.state if(stateVar.posOpt.nonEmpty && pos.isEmpty) ) {
      pos = stateVar.posOpt
    }
    for(method <- sc.methods if(method.method.posOpt.nonEmpty && pos.isEmpty)) {
      pos = method.method.posOpt
    }
    for(inv <- sc.invariants if inv.posOpt.nonEmpty && pos.isEmpty) {
      pos = inv.posOpt
    }
    if(sc.initializes.nonEmpty){
      for(exp <- sc.initializes.get.modifies if exp.posOpt.nonEmpty && pos.isEmpty) {
        pos = exp.posOpt
      }
      for(gaur <- sc.initializes.get.guarantees if(gaur.posOpt.nonEmpty && pos.isEmpty)) {
        pos = gaur.posOpt
      }
    }
    if(sc.integration.nonEmpty) {
      for(s <- sc.integration.get.specs if s.posOpt.nonEmpty && pos.isEmpty) {
        pos = s.posOpt
      }
    }
    if(sc.compute.nonEmpty) {
      for(m <- sc.compute.get.modifies if m.posOpt.nonEmpty && pos.isEmpty) {
        pos = m.posOpt
      }
      for(s <- sc.compute.get.specs if s.posOpt.nonEmpty && pos.isEmpty) {
        pos = s.posOpt
      }
      for(c <- sc.compute.get.cases if c.posOpt.nonEmpty && pos.isEmpty) {
        pos = c.posOpt
      }
      for(h <- sc.compute.get.handlers if h.port.posOpt.nonEmpty && pos.isEmpty) {
        pos = h.port.posOpt
      }
    }
    return pos
  ...
  def generateToolInstallSection(value: HashSMap[Cli.SireumHamrCodegenHamrPlatform.Type, Report]): Level = {
    var content: ST =
      st"""- Install Sireum and IVE by following the instructions at [https://github.com/sireum/kekinian#setup](https://github.com/sireum/kekinian#setup)
          |
          |- Install FMIDE
          |
  ...
  def generateArchitectureSection(value: HashSMap[Cli.SireumHamrCodegenHamrPlatform.Type, Report]): Level = {
    ...
    if(cand.nonEmpty) {
      ...
      if(report.symbolTable.nonEmpty){
        ...
        var systemProps = st"""|System: ${header}|
                              ||--|
                              ||"""
        ...
        for(thread <- threads) {
          ...
          val ais = GumboGen.getGclAnnexInfos(thread.path, symbolTable)
          assert(ais.size <= 1, "Can't attach more than 1 subclause to an AADL thread")

          var gumboPos: Option[ST] = None()
          if (ais.nonEmpty) {
            val h = createHeader("GUMBO contract", pos, "annex", aadlDir, rootDir)
            gumboPos = Some(st"|$h|")
            /*
            val sc = ais(0).annex
            getGumboPos(sc) match {
              case Some(pos) => gumboPos = createHeader("GUMBO Contract", pos, )
              case _ =>
            }*/
          }
    ...
    val title = "AADL Model with GUMBO Contracts"
    return ContentLevel(title, createTag(title), content)
  }
  ...
  def generatePlatformSections(reports: HashSMap[Cli.SireumHamrCodegenHamrPlatform.Type, Report]): ISZ[Level] = {
    ...
    for(e <- reports.entries){
      ...
      val fmideTitle = "From within FMIDE"
      val fmideI = ContentLevel(fmideTitle, createTag(fmideTitle), configuration)


      val runHamrScript = report.readmeDir.relativize(report.runHamrScript.get)
      val cliInst =
        st"""Run the following script: [${runHamrScript}](${runHamrScript})
            |
            |```
            |./$runHamrScript
            |```"""

      val commandLineTitle = "From the Command Line"
      val commandLineI = ContentLevel(commandLineTitle, createTag(commandLineTitle), cliInst)

      var title = s"Running HAMR Code Generation"
      subs = subs :+ SubLevel(title = title, tag = createTag(title), content = None(), subs = ISZ(fmideI, commandLineI))

      //subs = subs :+ ContentLevel(title, createTag(title), configuration)

      if(report.symbolTable.nonEmpty) {
        val symtable = report.symbolTable.get
        title = s"Slang Behavior Code"
        var locs: ISZ[ST] = ISZ()
        for(t <- sortThreads(symtable.getThreads(), symtable)) {

          val id = t.identifier
          platform match {
            case Cli.SireumHamrCodegenHamrPlatform.JVM =>
              val componentsDir = Os.path(report.options.outputDir.get) / "src" / "main" / "component"
              val slangSuffix = s"${id}.scala"
              hackyFind(componentsDir, slangSuffix) match {
                case Some(p) =>
                  val hasContracts = ops.StringOps(p.read).contains("Contract")
                  val iid = s"${id}${if(hasContracts) "*" else ""}"
                  locs = locs :+ createHyperLink(iid, report.readmeDir.relativize(p).value)
                case _ => halt(s" didn't find ${slangSuffix} in ${componentsDir}")
              }
            ...
            case Cli.SireumHamrCodegenHamrPlatform.SeL4 =>
              ...
              if(isVM) {
                ...
                val cdir = Os.path(report.options.slangOutputCDir.get) / "ext-c"
                hackyFind(cdir, csuffix) match {
                  case Some(p) =>
                    locs = locs :+ createHyperLink(id, report.readmeDir.relativize(p).value)
                  case _ => halt(s" didn't find ${csuffix} in $cdir")
                }
              }
            case _ => halt("NOOOO")
          }
      ...
      title = s"How to Build and Run the Generated Slang Program"
      subs = subs :+ ContentLevel(title, createTag(title), report.runInstructions)


      val proyekRoot = report.readmeDir / "hamr" / "slang"
      assert(proyekRoot.exists)

      val proyekRel = report.readmeDir.relativize(proyekRoot)
      val logikaLCIContents =
        st"""To have Logika verify the entire program, ``cd`` to the directory that contains this
            |readme and then run the following
            |
            |```
            |$$SIREUM_HOME/bin/sireum proyek logika --all ./${proyekRel}
            |```
            |
            ...
            |
            |```
            |$$SIREUM_HOME/bin/sireum proyek logika --timeout 3 --all ./${proyekRel}
            |```
            |"""
      val logikaCITitle = s"From the Command Line"
      val ciSub = ContentLevel(logikaCITitle, createTag(logikaCITitle), logikaLCIContents)

      val logikaIVETitle = s"From within IVE"
      val iveSub = ContentLevel(logikaIVETitle, createTag(logikaIVETitle),
        st"""In IVE open the Slang file you want to verify.  If the file is in Logika
      ...
      val logikaTitle = s"Verifying the Slang Program Using Logika"
      subs = subs :+ SubLevel(title = logikaTitle, tag = createTag(logikaTitle), content = None(), subs = ISZ(ciSub, iveSub ))
  ...
  def generateReport(title: String, reports: HashSMap[Cli.SireumHamrCodegenHamrPlatform.Type, Report]): ST = {
    ...
    val toolInstallSection: Level = generateToolInstallSection(reports)

    val archSection: Level = generateArchitectureSection(reports)

    val platformSections: ISZ[Level] = generatePlatformSections(reports)

    // we only have a JVM section
    assert(platformSections.size == 1)
    val subPlatformSections = (platformSections(0).asInstanceOf[SubLevel]).subs
    val toc: Level = generateTOC(toolInstallSection +: archSection +: subPlatformSections)
    ...
    val ret: ST = st"""# ${title}
                      ...
                      |
                      |${level2ST(toolInstallSection, 2, readmeExists)}
                      |
