...
@record class ReadmeGenerator_te4(o: Cli.SireumHamrCodegenOption, reporter: Reporter) {
  ...
  def genLinuxRunInstructions(readmeDir: Os.Path,
    ...
    val transpile = outputDir / "bin" / "transpile.cmd"
    val compile = bin / "compile.cmd"
    val run = bin / "run.sh"
...
object ReadmeTemplate_te4 {
  ...
  def replaceContent(tag: String, level: Level): Unit = {
    ...
    def doit(content: ST): Unit = {
      val _posStart = existingReadmeContents.stringIndexOf(start)
      if(_posStart > 0) {
        val posStart = _posStart + start.size // include the tag
